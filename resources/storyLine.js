var setStoryLineColors = function(clickedValue){
  var completedColor = "gray";

  console.log("setting story line colors");

  rect.style("fill", function(d){if(d.value <= clickedValue){return completedColor} return "white"})
  rect.style("stroke", function(d){if(d.value < clickedValue){return completedColor} return "white"} )
  lines.style("stroke", function(d){if(d.value < clickedValue){return completedColor} return "white"} )
...
.on("click", function(d){
  ...
  render(d.name);
    ...
    if(cfg.showLabels != false){
      ...
      svg.append("text")
         .text(startString).style("font-size", "70%").style("fill", "white")
